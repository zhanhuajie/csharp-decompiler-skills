        ...
        try:
            with open(cache_file, 'r', encoding='utf-8') as f:
                ...
                if cache_data.get('mtime') == assets_mtime:
                    return assets_path, cache_data.get('dlls', []), cache_data.get('groups', {})
        except Exception:
            ...
            for pkg_id, pkg_info in packages.items():
                ...
                if dll_rel_paths:
                    ...
                    for rel_path in dll_rel_paths:
                        full_dll_path = os.path.normpath(os.path.join(pkg_full_path, rel_path))
                        if os.path.exists(full_dll_path) and full_dll_path not in package_groups[pkg_id]:
        ...
        for ref in ref_elements:
            ...
            if hint_path is not None and hint_path.text:
                ...
                if not os.path.isabs(path):
                    path = os.path.abspath(os.path.join(os.path.dirname(project_path), path))
...
def check_dotnet_script_installed():
    ...
    try:
        result = subprocess.run(['dotnet', 'script', '--version'], capture_output=True, text=True)
...
def fast_scan_dlls(dlls, target_type, namespace_filter=None):
    ...
    if not os.path.exists(scanner_script):
        print(f"Error: Could not find the C# metadata scanner script at {scanner_script}.", file=sys.stderr)
        return []
        
    if not check_dotnet_script_installed():
        print("Error: 'dotnet-script' is not installed. Please install it by running: dotnet tool install -g dotnet-script", file=sys.stderr)
        return []
...
def decompile_type(dll_path, full_type_name):
    ...
    try:
        result = subprocess.run(['ilspycmd', '-t', full_type_name, dll_path], capture_output=True, text=True)
...
def main():
    parser = argparse.ArgumentParser(description="Lookup C# class definitions in project references.")
    parser.add_argument("symbol", help="The class name to lookup (e.g., Asset)")
    parser.add_argument("--project", help="Path to the .csproj file to scan for references.")
    parser.add_argument("--dll", help="Optional specific DLL path to search in.")
    parser.add_argument("--list-refs", action="store_true", help="List all resolved DLL references before searching.")
    parser.add_argument("--full", action="store_true", help="Print full decompiled output without truncating.")
    parser.add_argument("--namespace", help="Filter results by namespace prefix (e.g., Autodesk).")
    
    args = parser.parse_args()
    
    if not args.project and not args.dll:
        print("Error: No search source provided. Use --project <path> or --dll <path>.", file=sys.stderr)
        return
    ...
    if args.project and not dlls_to_check:
        print(f"Warning: No DLL references found in project '{args.project}'.", file=sys.stderr)
        print("If this is an SDK-style project, ensure you have run 'dotnet restore'.", file=sys.stderr)
        return
    ...
    if args.list_refs:
        ...
        local_dlls = [d for d in dlls_to_check if not any(d in g_dlls for g_dlls in asset_groups.values())]
        if local_dlls:
            print(f"  Local/Explicit References ({len(local_dlls)} dlls)")
        print("")

    print(f"Searching for symbol '{args.symbol}' in {len(dlls_to_check)} references using fast metadata scanner...")
    ...
    for full_name, dll in matches:
        ...
        print("\n".join(lines[:limit]))
        if len(lines) > limit:
            print(f"... ({len(lines) - limit} lines truncated, use --full to see all)")
